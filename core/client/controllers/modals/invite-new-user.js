            ...
            this.store.find('user').then(function (result) {
                ...
                if (invitedUser) {
                    if (invitedUser.get('status') === 'invited' || invitedUser.get('status') === 'invited-pending') {
                        self.notifications.showWarn('Un usuario con ese email ya fue invitado.');
                    } else {
                        self.notifications.showWarn('Un usuario con ese email ya existe.');
                    }
                } else {
                    ...
                    newUser.save().then(function () {
                        var notificationText = 'Invitacion enviada! (' + email + ')';

                        // If sending the invitation email fails, the API will still return a status of 201
                        // but the user's status in the response object will be 'invited-pending'.
                        if (newUser.get('status') === 'invited-pending') {
                            self.notifications.showWarn('La invitacion no pudo ser enviada.  Intente reenviarla.');
                        } else {
